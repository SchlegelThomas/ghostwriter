...
7. Add targeted tests at each layer.
8. Run the checkpoint verification ladder.
9. Perform the cumulative coherence review.
10. Update docs and the record log immediately.
11. If commits are authorized, create one conventional checkpoint commit.
...
- **Regression:** every prior accepted feature still works in targeted and browser journeys.
...
3. Focused Playwright journey for the checkpoint.
4. `pnpm verify`.
5. Full `pnpm test:e2e` at visual milestones, migrations, or cross-surface changes.
6. `git diff --check` and changed-document link/structure validation.

Do not mark acceptance complete from typecheck alone. Record exact evidence and test counts.
