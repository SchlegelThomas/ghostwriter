...
- Manuscript parts may store optional `summary` text (`manuscript_parts.summary`, migration 0010)
  edited via `part.update`, parallel to chapter summaries.
- Writer profiles may store optional `publishing` JSON (contact, mailing address, bio, representation)
...
- Catalog agents (Idea Midwife, Pacing Doctor, …) run via
  `POST /api/projects/{projectId}/agent/catalog-runs`, producing typed proposals
  (`catalog-memo-v1` or `pacing-findings-v1`) with [ADR 0014](adr/0014-entity-draft-proposals.md)
  primary targets. List summaries include a structured `preview` for Drafts cards. Writer steering
  sits on top of built-in playbooks (`/catalog-playbooks`); product constraints stay authoritative
  (see ADR 0011).
